...
########### NQUEEN PROBLEM: LOCAL SEARCH v2 
### F.Mazza M63001338
...
# Inizializzo una tabella di dimensioni NxN, all'inizio tutti valori zero, 
# significa che tutti gli scambi (pos1,pos2) sono consentiti.
def TabuInit(N):
    Tabu = [[0 for i in range(N)] for j in range(N)]
    return Tabu

# Decremento di 1 tutte le posizioni alla fine di ogni turno.
def TabuUpdate(N,Tabu):
    for i in range(N):
        for j in range(i,N):
            if(Tabu[i][j] > 0):
                Tabu[i][j] = Tabu[i][j]-1
...
def localSearch(N,Tabu):
    ...
    for i in range(max_iter):
        ...
        for j in range(len(current_state)):
            ...
            # CHECK nuovo stato non deve essere tabu: per verificarlo controllo la tabella
            # in posizione [pos1,pos2], se il valore è >0 la mossa è tabu, e passo direttamente
            # alla sostituzione successiva (break)

            if(Tabu[pos1][j] > 0):
                print("Soluzione TABU!")
                break
            ...
            # Ogni volta che osservo una nuova soluzione, impongo che sia impossibile, 
            # per un certo numero di turni, effettuare lo stesso scambio. 
            # (nota: migliorare il tuning sul numero di turni).

            if (next_cost <= current_cost):
                current_state = next_state
                current_cost = next_cost
                print("Aggiorno Tabu in posizione:",pos1,j)
                Tabu[pos1][j] = Tabu[pos1][j]+2

            
            # Se, ad esempio, ho incontrato una soluzione peggiorativa, posso decidere
            # di agire sulla matrice, impedendo di osservarla nuovamente per un certo numero
            # di turni (valore diverso rispetto ad una soluzione migliorativa osservata).
            
            if (next_cost > current_cost):
                print("Aggiorno Tabu in posizione:",pos1,j)
                Tabu[pos1][j] = Tabu[pos1][j]+5
    

        # Terminato lo swap per il valore random selezionato, aggiorno la tabella
        # delle mosse Tabu decrementando di 1 tutti i valori maggiori di 0
        print("Tabu update...")
        TabuUpdate(N,Tabu)
        print(Tabu)
...
# di attacco reciproco, il costo aumenta di 1 (numero di conflitti). Quando il costo della soluzione candidata è zero,
